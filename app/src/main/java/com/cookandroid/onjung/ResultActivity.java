...
public class ResultActivity extends AppCompatActivity
    ...
    CourseHandler2 courseHandler2 = new CourseHandler2();
    Message message2 = courseHandler2.obtainMessage();
    Bundle bundle2 = new Bundle();
    CourseHandler3 courseHandler3 = new CourseHandler3();
    Message message3 = courseHandler3.obtainMessage();
    ...
    class CourseHandler2 extends Handler {
        @Override
        public void handleMessage(@NonNull Message msg) {
            super.handleMessage(msg);

            Bundle bundle = msg.getData();
            String course2 = bundle.getString("courseInfo2");
/*
            if (flag == 2) {
                System.out.println("로그 6: " + course2);
                courseInfo.setText(course2);
            }
 ...
 */
    ...
    class CourseHandler3 extends Handler {
        @Override
        public void handleMessage(@NonNull Message msg) {
            super.handleMessage(msg);

            Bundle bundle = msg.getData();
            String course3 = bundle.getString("courseInfo3");
/*
            if (flag == 3) {
                System.out.println("로그 6: " + course3);
                courseInfo.setText(course3);
            }
 ...
 */
    ...
    class HttpConnectorTime extends Thread {
        ...
        public HttpConnectorTime() {
            try {
                data = new JSONObject();
                // 보낼 데이터를 넣어준다 data.put("a",a);
                data.put("appKey", API_Key);
                data.put("startX", (Double) 126.8516124); // 출발지 경도
                data.put("startY", (Double) 37.5307786); // 출발지 위도
                data.put("endX", (Double) 126.8516124); // 도착지 경도
                data.put("endY", (Double) 37.5307786); // 도착지 위도
                data.put("startName", "home");
                data.put("endName", "home");
                data.put("searchOption", 10);
                data.put("passList", "126.8774698,37.54228826_126.856902,37.530115");

                url = new URL("https://apis.openapi.sk.com/tmap/routes/pedestrian?version=1&callback=");
                conn = (HttpURLConnection) url.openConnection();
                conn.setRequestMethod("POST");
                ...
                e.printStackTrace();
                System.out.println("로그: 소요시간 POST 예외 발생");
            }
        }
